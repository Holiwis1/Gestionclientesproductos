...
public class SistemaPago {
	...
	/**
	 * Constructor de la clase SistemaPago.
	 * 
		...
		if (pagoC >= cuenta) {
			...
			for (int i = 0; i < denominations.length; i++) {
				...
				if (numDenomination > 0) {
					System.out.println(numDenomination + " " + (denominations[i] >= 100 ? "billete/s" : "moneda/s")
							+ " de " + denominations[i] / 100.0);
				}
	...
	public void cuentaCorriente() {
		...
		if (cuenta.matches("^ES[0-9]{2}[0-9]{20}$") == true) {
			System.out.println("Formato correcto");
		}

		if (cuenta.matches("^ES[0-9]{2}[0-9]{20}$") == false) {
			do {
				System.out.println("Formato erroneo , introduzca otra vez su cuenta :");
				cuenta = sc.nextLine();
			} while (cuenta.matches("^ES[0-9]{2}[0-9]{20}$") == false);
			System.out.println("Formato correcto");
		}

	}
	/**
	 * Método para validar un número decimal ingresado por el usuario.
	...
	public void tarjeta() {
		...
		if (tarjeta.matches("^[0-9]{16}$") == true) {
			System.out.println("Formato correcto");
		}

		if (tarjeta.matches("^[0-9]{16}$") == false) {
			do {
				System.out.println("Formato erroneo , introduzca otra vez su tarjeta :");
				tarjeta = sc.nextLine();
			} while (tarjeta.matches("^[0-9]{16}$") == false);
			System.out.println("Formato correcto");
		}
